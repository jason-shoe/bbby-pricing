...
class Navigation extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:5000/get_table/0/none/none/1000')
        .then(response => {
            this.setState({
                table: response.data
            })
    ...
    handleQueryChange = (event) => {
        if(event.keyCode == 13){
            ...
            if(event.target.value == ""){
                axios.get('http://localhost:5000/get_table/0/' + this.state.sort_method + '/' + this.state.filter_method + '/1000')
                .then(response => {
                    this.setState({
                        table: response.data
                    })
                ...
                axios.get('http://localhost:5000/get_table/' + event.target.value+'/' + this.state.sort_method + '/' + this.state.filter_method + '/1000')
                .then(response => {
                    this.setState({
                        table: response.data
                    })
    ...
    handleSortChange = (event) => {
        ...
        axios.get('http://localhost:5000/get_table/' + this.state.query + '/' + event.target.value + '/' + this.state.filter_method + '/500')
        .then(response => {
            this.setState({
                table: response.data
            })
    ...
    handleFilterChange = (event) => {
        ...
        axios.get('http://localhost:5000/get_table/' + this.state.query + '/' + this.state.sort_method + '/' + event.target.value +'/500')
        .then(response => {
            this.setState({
                table: response.data
            })
                    ...
                    <div className={styles.container}>
                        ...
                        <div style={{height:'500px',overflowY:'auto',width:'100%', borderTop:'1px solid black',borderBottom:'1px solid black'}}>
                            <table style={{width:'100%',borderCollapse:'collapse'}}>
                                ...
                                {Object.keys(table).splice(4, Object.keys(table).length).map(keyTwo => 
